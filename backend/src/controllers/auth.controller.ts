 ...
 * /auth/facebook:
 *   post:
 *     summary: Login or register with Facebook access token
 *     tags: [Auth]
 *     requestBody:
 *       required: true
 *       content:
...
export const loginWithFacebook = async (req: Request, res: Response) => {
  try {
    ...
    if (!accessToken) {
      return errorResponse(res, 'AUTH_013', 'Facebook access token is required', 400);
    }
    ...
    if (!FACEBOOK_APP_ID || !FACEBOOK_APP_SECRET) {
      return errorResponse(res, 'SERVER_002', 'Facebook credentials not configured', 500);
    }
    ...
    let debugResponse;
    try {
      debugResponse = await axios.get(debugUrl);
    } catch (error: any) {
      return errorResponse(res, 'AUTH_014', 'Invalid Facebook token', 400);
    }

    const tokenData = debugResponse.data?.data;
    if (!tokenData || !tokenData.is_valid || tokenData.app_id !== FACEBOOK_APP_ID) {
      return errorResponse(res, 'AUTH_014', 'Invalid Facebook token', 400);
    }
    ...
    try {
      ...
      return errorResponse(res, 'AUTH_015', 'Failed to fetch Facebook profile', 400);
    }

    const facebookProfile = profileResponse.data;
    const facebookId = facebookProfile.id;
    const name = facebookProfile.name;
    const email = facebookProfile.email || null;

    // Check if user exists with this Facebook ID
    let user = await User.findOne({ 'socialIds.facebook': facebookId });
    ...
    // Generate JWT tokens
    const jwtPayload = { userId: user._id, phone: user.phone, roles: user.roles };
    const jwtAccessToken = signToken(jwtPayload, JWT_SECRET, JWT_EXPIRES);
    const jwtRefreshToken = signToken(jwtPayload, REFRESH_TOKEN_SECRET, REFRESH_TOKEN_EXPIRES);
    ...
    return errorResponse(res, 'SERVER_001', error.message || 'Facebook login failed', 500);
  }
};
