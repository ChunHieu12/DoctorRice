...
import { IoTConnection, IIoTConnection } from '../models/IoTConnection';
    ...
    if (existingConnection) {
      const updatedConnection = await IoTConnection.findByIdAndUpdate(
        existingConnection._id,
        {
          ...
          firebaseDbUrl: existingConnection.firebaseDbUrl || 'https://rice-813b5-default-rtdb.firebaseio.com',
          status: 'active',
      ...
      if (!updatedConnection) {
        logger.warn(`⚠️ Failed to update existing IoT connection ${existingConnection._id}, falling back to current document`);
